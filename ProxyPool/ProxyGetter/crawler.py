...
import json
from ProxyPool.ProxyGetter.utils import get_page
...
class Crawler(object, metaclass=ProxyMetaclass):
    def get_proxies(self, callback, count):
        proxies = []
    ...
    # def crawl_zhima_proxy(self, count):
    #     """
    #     获取芝麻代理
    #     :return: 代理
    #     """
    #     url = 'http://webapi.http.zhimacangku.com/getip?num={}&type=2&pro=0&city=0&yys=0&port=1&pack=69876&ts=0' \
    #           '&ys=0&cs=0&lb=1&sb=0&pb=4&mr=1&regions='.format(str(count))
    #     html = get_page(url)
    #     if html:
    ...
    def crawl_adsl_proxy(self, count):
        """
        获取拨号代理
        :return: 代理
        """
        url = 'http://47.104.235.235:8000/proxies'
        html = get_page(url)
        if html:
