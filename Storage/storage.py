...
class Dao(object):
    ...
    def __saveMediaToHbase(self, media_url, content, item, type, contype):
        ...
        while True:
            start_time = time.time()
            try:
                resp = requests.post(url=url, headers=headers, data=form_data, timeout=(5, 10)).content.decode('utf-8')
                respon = json.loads(resp)
                if respon['resultCode'] == 0:
...
def hbase_test():
    ...
    resp = requests.post(url=url, data=form_data, timeout=(5, 10)).content.decode('utf-8')
    respon = json.loads(resp)
    if respon['resultCode'] == 0:
        print(resp)
