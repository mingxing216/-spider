...
logger.add("/opt/Log/SheHuiKeXue/期刊论文_retry_queue_{time}.log",
...
class SpiderMain(BastSpiderMain):
    ...
    def start(self):
        while 1:
            # 查询redis队列中任务数量
            url_number = self.dao.select_task_number(key=config.REDIS_ZHEXUESHEHUIKEXUE_PAPER_RETRY)
            if url_number <= config.MAX_QUEUE_REDIS/10:
                ...
                # print(new_task_list)
                # 队列任务
                self.dao.queue_tasks_from_mysql_to_redis(key=config.REDIS_ZHEXUESHEHUIKEXUE_PAPER_RETRY, data=new_task_list)
            else:
