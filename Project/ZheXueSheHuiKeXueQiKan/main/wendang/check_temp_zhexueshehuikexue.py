...
class SpiderMain(BastSpiderMain):
    ...
    def handle(self, task_data_list):
        pdf_list = []
        ...
        if info_list:
            for i in range(len(info_list)):
                if b'm:content' not in info_list[i].keys():
                    # 更新全文种子错误信息及状态
                    msg = '全文content字段缺失, url: {}'.format(pdf_list[i]['pdf_url'])
                    logger.warning(msg)
                    data = {
                        'del': '0',
                        'msg': msg,
                ...
                elif not info_list[i].get(b'm:content'):
                    # 更新全文种子错误信息及状态
                    msg = '全文content字段无值, url: {}'.format(pdf_list[i]['pdf_url'])
                    logger.warning(msg)
                    data = {
                        'del': '0',
                        'msg': msg,
                    ...
                    info_bs64 = info_list[i].get(b'm:content').decode('utf-8')
                    # print(info_bs64)
                    ...
                    if not is_value:
                        # 更新全文种子错误信息及状态
                        msg = 'not PDF, url: {}'.format(pdf_list[i]['pdf_url'])
                        logger.warning(msg)
                        data = {
                            'del': '0',
                            'msg': msg,
                        ...
                        # 记录全文正确信息
                        msg = 'is PDF, url: {}'.format(pdf_list[i]['pdf_url'])
                        logger.info(msg)
                        data = {
                            'del': '3',
                            'msg': msg,
            ...
            for i in range(len(info_list)):
                msg = '全文数据获取失败, url: {}'.format(pdf_list[i]['pdf_url'])
                logger.warning(msg)
                data = {
                    'del': '0',
                    'msg': msg,
    ...
    def run(self):
        ...
        while True:
            ...
            task_list = self.dao.get_task_from_redis(key=config.REDIS_ZHEXUESHEHUIKEXUE_PAPER, count=10)
            if task_list:
                try:
                    data_list = []
                    ...
                    # 获取全文数据并判断是否正确
                    self.handle(task_data_list=data_list)
