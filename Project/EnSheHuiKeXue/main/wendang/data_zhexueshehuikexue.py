...
class SpiderMain(BastSpiderMain):
    ...
    def __get_resp(self, url, method, s=None, data=None, cookies=None, referer=None, ranges=None, user=None):
        ...
        # 发现验证码，请求页面3次
        for _i in range(3):
            resp = self.download_middleware.get_resp(url, method, s=s, data=data,
    ...
    # 获取文档实体字段
    def document(self, pdf_dict, save_data):
        ...
        # ============= 文档实体数据 ================
        # 获取标题
        save_data['title'] = pdf_dict['bizTitle']
        # 文档内容
        size = len(pdf_content)
        # size = len(pdf_content)
        save_data['label_obj'] = server.getDocs(pdf_dict, size)
        # 获取来源网站
        save_data['source_website'] = ""
        # 关联论文
        save_data['rela_paper'] = pdf_dict['relEsse']
    ...
    def handle(self, task_data, save_data):
        ...
        if pdf_url:
            ...
            pdf_dict['relPics'] = server.rela_document(doc_url, doc_key, doc_sha)

            # 删除响应数据缓存
            server.clear()
            # 存储文档实体及文档本身
            suc = self.document(pdf_dict=pdf_dict, save_data=save_data)
            if not suc:
                return
        else:
            return
        ...
        # ===================公共字段
        # url
        save_data['url'] = pdf_dict['url']
        # 生成key
        save_data['key'] = pdf_dict['key']
        # 生成sha
        save_data['sha'] = hashlib.sha1(save_data['key'].encode('utf-8')).hexdigest()
        # 生成ss ——实体
        save_data['ss'] = '文档'
        # 生成es ——栏目名称
        save_data['es'] = '论文'
