...
LOGNAME = '<知网_发明授权_task>'  # LOG名
NAME = '知网_发明授权_task'  # 爬虫名
...
class SpiderMain(BastSpiderMain):
    def __init__(self):
        super().__init__()
        # 入口页url
        self.index_url = 'http://kns.cnki.net/kns/brief/result.aspx?dbPrefix=SCPD_SQ'
        # 指定年的主页url
        self.getYearIndexUrl = 'http://kns.cnki.net/kns/brief/brief.aspx?action=5&dbPrefix=SCPD_SQ&PageName=ASP.brief_result_aspx&Param=%e5%b9%b4+%3d+%27{}%27&isinEn=0&RecordsPerPage=50'
        # 获取最小级分类号
            ...
            for first_url in first_list:
                ...
                if second_list:
                    for second_url in second_list:
                        ...
                        # print(category_list)
                        # # 进入队列
                        # self.dao.QueueJobTask(key=config.REDIS_CATEGORY, data=category_list)
                        if category_list:
