...
class SpiderMain(BastSpiderMain):
    ...
    def start(self):
        while 1:
            # 查询redis队列中任务数量
            url_number = self.dao.selectTaskNumber(key=config.REDIS_WG_PATENT)
            if url_number == 0:
                LOGGING.info('redis已无任务，准备开始队列任务。')
                # 获取任务
                new_task_list = self.dao.getNewTaskList(table=config.MYSQL_PATENT, ws='中国知网', es='外观专利', count=2000)
                # print(new_task_list)
                # 队列任务
                self.dao.QueueTask(key=config.REDIS_WG_PATENT, data=new_task_list)
            else:
