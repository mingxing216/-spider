...
LOGNAME = '<知网_实用新型_queue>'  # LOG名
NAME = '知网_实用新型_queue'  # 爬虫名
...
class SpiderMain(BastSpiderMain):
    ...
    def start(self):
        while 1:
            # 查询redis队列中任务数量
            url_number = self.dao.select_task_number(key=config.REDIS_XX_PATENT)
            if url_number == 0:
                LOGGING.info('redis已无任务，准备开始队列任务。')
                # 获取任务
                new_task_list = self.dao.get_task_list_from_mysql(table=config.MYSQL_PATENT, ws='中国知网', es='实用新型', count=2000)
                # print(new_task_list)
                # 队列任务
                self.dao.queue_tasks_from_mysql_to_redis(key=config.REDIS_XX_PATENT, data=new_task_list)
            else:
