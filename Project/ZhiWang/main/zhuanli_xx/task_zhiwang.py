...
LOGNAME = '<知网_实用新型_task>'  # LOG名
NAME = '知网_实用新型_task'  # 爬虫名
...
class SpiderMain(BastSpiderMain):
    def __init__(self):
        super().__init__()
        # 入口页url
        self.index_url = 'https://kns.cnki.net/kns/brief/result.aspx?dbprefix=SCPD_XX'
        # 指定年的主页url
        self.getYearIndexUrl = 'https://kns.cnki.net/kns/brief/brief.aspx?action=5&dbPrefix=SCPD_XX&PageName=ASP.brief_result_aspx&Param=%e5%b9%b4+%3d+%27{}%27&RecordsPerPage=50'
        # 获取最小级分类号
            ...
            for first_url in first_list:
                ...
                if second_list:
                    for second_url in second_list:
                        second_resp = self.__getResp(url=second_url,
                                                     method='GET')
                        if not second_resp:
                        ...
                        # print(category_list)
                        # 进入队列
                        self.dao.queue_tasks_to_redis(key=config.REDIS_XX_CATEGORY, data=category_list)
                        if category_list:
    ...
    def getProfile(self, resp, category, year):
        ...
        while True:
            ...
            for url in next_urls:
                ...
                # 存入数据库
                self.dao.save_task_to_mysql(table=config.MYSQL_PATENT, memo=url, ws='中国知网', es='实用新型')
    ...
    def run(self, category):
        ...
        while True:
            ...
            if not self.cookie_dict:
                # 队列一条任务
                self.dao.queue_one_task_to_redis(key=config.REDIS_XX_CATEGORY, data=category)
                return
            # 获取时间列表
    ...
    def start(self):
        ...
        while 1:
            # 获取任务
            category_list = self.dao.get_task_from_redis(key=config.REDIS_XX_CATEGORY,
                                                         count=1,
                                                         lockname=config.REDIS_XX_CATEGORY_LOCK)
            # print(category_list)
