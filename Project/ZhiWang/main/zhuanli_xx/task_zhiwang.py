...
class SpiderMain(BastSpiderMain):
            ...
            for first_url in first_list:
                ...
                if second_list:
                    for second_url in second_list:
                        ...
                        # print(category_list)
                        # 进入队列
                        self.dao.QueueJobTask(key=config.REDIS_XX_CATEGORY, data=category_list)
                        if category_list:
    ...
    def getProfile(self, resp, category, year):
        ...
        while True:
            ...
            for url in next_urls:
                ...
                # 存入数据库
                self.dao.saveTaskToMysql(table=config.MYSQL_PATENT, memo=url, ws='中国知网', es='实用新型')
    ...
    def start(self):
        ...
        while 1:
            # 获取任务
            category_list = self.dao.getTask(key=config.REDIS_XX_CATEGORY,
                                         count=1,
