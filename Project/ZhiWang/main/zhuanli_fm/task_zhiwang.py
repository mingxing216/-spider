...
class SpiderMain(BastSpiderMain):
            ...
            for first_url in first_list:
                ...
                if second_list:
                    for second_url in second_list:
                        ...
                        # print(category_list)
                        # 进入队列
                        self.dao.queue_tasks_to_redis(key=config.REDIS_FM_CATEGORY, data=category_list)
                        if category_list:
    ...
    def getProfile(self, resp, category, year):
        ...
        while True:
            ...
            for url in next_urls:
                ...
                # 存入数据库
                self.dao.save_task_to_mysql(table=config.MYSQL_PATENT, memo=url, ws='中国知网', es='发明公开专利')
    ...
    def run(self, category):
        ...
        while True:
            ...
            if not self.cookie_dict:
                # 队列一条任务
                self.dao.queue_one_task_to_redis(key=config.REDIS_FM_CATEGORY, data=category)
                return
            # 获取时间列表
    ...
    def start(self):
        ...
        while 1:
            # 获取任务
            category_list = self.dao.get_task_from_redis(key=config.REDIS_FM_CATEGORY,
                                                         count=1,
