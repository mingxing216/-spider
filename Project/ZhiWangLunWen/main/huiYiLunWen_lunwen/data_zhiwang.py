...
class SpiderMain(BastSpiderMain):
    ...
    def handle(self, task, save_data):
        ...
        # 获取时间
        shijian = self.server.get_more_fields(article_html, '时间')
        if shijian:
    ...
    def start(self):
        while 1:
            # 获取任务
            task_list = self.dao.get_task_from_redis(key=config.REDIS_HUIYI_PAPER, count=50, lockname=config.REDIS_HUIYI_PAPER_LOCK)
            # print(task_list)
