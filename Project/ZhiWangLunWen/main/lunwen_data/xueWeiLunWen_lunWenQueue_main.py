...
class BastSpiderMain(object):
    def __init__(self):
        ...
        self.dao = dao.XueWeiLunWen_LunWenDataDao(logging=LOGGING,
...
class SpiderMain(BastSpiderMain):
    ...
    def start(self):
        while 1:
            # 查询redis队列中任务数量
            url_number = self.dao.selectTaskNumber(key=config.REDIS_XUEWEILUNWEN_LUNWEN)
            if url_number == 0:
                LOGGING.info('redis已无任务，准备开始队列任务。')

                # 获取任务
                new_task_list = self.dao.getNewTaskList(table=config.MYSQL_LUNWEN, where=config.XUEWEILUNWEN_LUNWEN_MAIN,
                                                        count=2000)
                LOGGING.info('已从Mysql获取到{}个任务'.format(len(new_task_list)))

                # 队列任务
                self.dao.QueueTask(key=config.REDIS_XUEWEILUNWEN_LUNWEN, data=new_task_list)
