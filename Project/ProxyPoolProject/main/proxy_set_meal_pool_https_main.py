...
log_file_dir = 'proxy_set_mail_pool_https_main'  # LOG日志存放路径
...
def maintainProxyPool():
    ...
    代理池维护
    '''
    redis_key = settings.REDIS_PROXY_KEY_HTTPS
    redis_proxy_number = settings.REDIS_PROXY_NUMBER_HTTPS
        ...
        if proxy_number < int(redis_proxy_number):
            ...
            if proxys is not None:
                for proxy_dict in proxys:
                    proxy = 'https://%s:%s' % (proxy_dict['ip'], proxy_dict['port'])
                    # # 检测代理是否高匿
