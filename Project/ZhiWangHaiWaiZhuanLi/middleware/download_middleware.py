...
class UrlDownloader(downloader.BaseDownloaderMiddleware):
    ...
    def __one(self, category, country):
    ...
    def __two(self, category, country):
    ...
    def __third(self, category, country):
    ...
    def getTimeListResp(self, category, country):
        self.__one(category=category, country=country)
        self.__two(category=category, country=country)
        self.__third(category=category, country=country)
    ...
    def getIndexHtml(self, year):
        url = 'http://kns.cnki.net/kns/brief/brief.aspx?action=5&dbPrefix=SOPD&PageName=ASP.brief_result_aspx&Param=%E5%B9%B4%20=%20%27{}%27'.format(year)
        # 设置请求头
        headers = {
            'Cookie': COOKIE,
            'User-Agent': user_agent_u.get_ua()
