...
LOGNAME = 'Techstreet_标准_task'  # LOG名
NAME = 'Techstreet_标准_task'  # 爬虫名
...
class SpiderMain(BastSpiderMain):
    def __init__(self):
        super().__init__()
        # 入口种子
        self.index_url = 'https://www.techstreet.com/publishers/list'
        # 列表页种子存放列表
    ...
    def getCatalog(self):
        ...
        #     f.write(index_text)
        # 获取列表中的所有出版商url
        publish_url_list = self.server.getPublisherUrl(resp=index_text)
        ...
        for publish_url in publish_url_list:
            publish_resp = self.__getResp(func=self.download_middleware.getResp,
                                        ...
                                        mode='GET')
            if not publish_resp:
                LOGGING.error('出版商页面响应获取失败, url: {}'.format(publish_url))
                return
    ...
    def run(self, catalog_url):
        ...
        for url in first_urls:
            ...
            # 存入数据库
            self.dao.saveTaskToMysql(table=config.MYSQL_STANDARD, memo=url, ws='techstreet', es='标准')
