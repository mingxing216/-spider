...
LOGNAME = 'Techstreet_标准_data'  # LOG名
NAME = 'Techstreet_标准_data'  # 爬虫名
...
class SpiderMain(BastSpiderMain):
    ...
    def standard(self, save_data, select, html, url, sha):
        ...
        # 获取标准发布组织
        save_data['biaoZhunFaBuZuZhi'] = self.server.getBiaoZhunFaBuZuZhi(select)
        # 获取摘要
        save_data['zhaiYao'] = self.server.getZhaiYao(html)
        # 获取版本
        save_data['banBen'] = self.server.getField(select, 'Edition')
        # 获取出版日期
        save_data['chuBanRiQi'] = self.server.getRiqi(select)
        # 获取ISBN
        save_data['ISBN'] = self.server.getField(select, 'ISBN')
        # 获取页数
