...
LOGNAME = 'Nasa_会议论文_data'  # LOG名
NAME = 'Nasa_会议论文_data'  # 爬虫名
...
class SpiderMain(BastSpiderMain):
    ...
    def lunWen(self, save_data, selector, resp, url, sha, es):
        ...
        # 获取会议信息
        save_data['huiYiXinXi'] = self.server.getField(selector, 'Meeting Information')
        # 获取主办方
        save_data['zhuBanFang'] = self.server.getMoreField(selector, 'Meeting Sponsor')
        # 获取赞助商
    ...
    def start(self):
        while 1:
            # 获取任务
            task_list = self.dao.getTask(key=config.REDIS_HUIYI_PAPER, count=10, lockname=config.REDIS_HUIYI_PAPER_LOCK)
