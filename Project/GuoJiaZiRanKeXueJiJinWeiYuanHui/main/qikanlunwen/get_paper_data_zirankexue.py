...
logger.add("/opt/Log/ZiRanKeXue/论文_test_data_{time}.log",
...
class SpiderMain(BastSpiderMain):
    ...
    def handle(self, task_data):
        ...
        if paper_url:
            ...
            if info_dict:
                # # 更新全文种子错误信息及状态
                # msg = 'hbase有该实体数据, sha: {}'.format(paper_url)
                # logger.info(msg)
                # data = {
                #     'del': '5',
                ...
                logger.info('论文实体数据写入文本成功, url: {}'.format(paper_url))

            else:
                logger.warning('无论文实体数据, url: {}'.format(paper_url))
