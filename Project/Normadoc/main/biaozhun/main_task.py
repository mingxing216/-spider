...
LOGNAME = 'Normadoc_标准_task'  # LOG名
NAME = 'Normadoc_标准_task'  # 爬虫名
...
class SpiderMain(BastSpiderMain):
    def __init__(self):
        super().__init__()
        # 入口种子
        self.index_url = 'http://www.normadoc.com/english/'
        # 列表页种子存放列表
    ...
    def getCatalog(self):
        ...
        for publish_url in publish_url_list:
            publish_resp = self.__getResp(func=self.download_middleware.getResp,
                                        ...
                                        mode='GET')
            if not publish_resp:
                LOGGING.error('国际组织响应获取失败, url: {}'.format(publish_url))
                return

            publish_text = publish_resp.text

            # 获取列表页
            catalog_urls = self.server.getCatalogUrlList(resp=publish_text)
            if not catalog_urls:
                return
            # print(catalog_urls)
    ...
    def run(self, catalog_url):
        ...
        for url in first_urls:
            ...
            # 存入数据库
            self.dao.saveTaskToMysql(table=config.MYSQL_STANDARD, memo=url, ws='normadoc', es='标准')
