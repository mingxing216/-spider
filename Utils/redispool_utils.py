...
def queue_spop(**kwargs):
    ...
    :param kwargs: 
    :return: 被删除的元素
    使用案例 data = queue_spop(redis_client=redis_client, key='comlieted', lockname='spop_demo')
...
def queue_spops(**kwargs):
    ...
    :param kwargs: 
    :return: 被删除的集合元素
    使用案例 data = queue_spops(redis_client=redis_client, key='comlieted', lockname='srandmember_demo', count=3)
    '''
    return_data = []
    redis_client = kwargs['redis_client']
    try:
